...
class MahjongGBEnv():
    ...
    def __init__(self, config):
        ...
        assert issubclass(
            self.agentclz, MahjongGBAgent), "ageng_clz must be a subclass of MahjongGBAgent!"
    ...
    def reset(self, prevalentWind=-1, tileWall=''):
        ...
        if self.duplicate:
            self.tileWall = [
                self.tileWall[i * 34: (i + 1) * 34] for i in range(4)]
    ...
    def step(self, action_dict):
        try:
            if self.state == 0:
                ...
                responses = {i: self.agents[i].action2response(
                    action_dict[self.agent_names[i]]) for i in range(4) if i != self.curPlayer}
                ...
                for j in range(1, 4):
                    ...
                    for j in range(1, 4):
                        ...
                        if t[i][0] == 'Chi' and not self.wallLast:
                            ...
                            if self.wallLast:
                                # A draw
                                self.obs = {i: self.agents[i].request2obs(
                                    'Huang') for i in range(4)}
                                self.reward = [0, 0, 0, 0]
                                self.done = True
                                self.status = NOWINNER
                            else:
                                # Next player
            ...
            player = e.args[0]
            self.obs = {i: self.agents[i].request2obs(
                'Player %d Invalid' % player) for i in range(4)}
            self.reward = [10] * 4
            self.reward[player] = -30
            self.done = True
            self.status = ERROR
        return self._obs(), self._reward(), self._done(), self.status
    ...
    def _discard(self, player, tile):
        ...
        self.obs = {i: self.agents[i].request2obs(
            'Player %d Play %s' % (player, tile)) for i in range(4) if i != player}
    ...
    def _kong(self, player, tile):
        ...
        self.packs[player].append(
            ('GANG', tile, (player + 4 - self.curPlayer) % 4))
    ...
    def _pung(self, player, tile):
        ...
        # offer: 0 for self, 123 for up/oppo/down
        self.packs[player].append(
            ('PENG', tile, (player + 4 - self.curPlayer) % 4))
        ...
        self.obs = {player: self.agents[player].request2obs(
            'Player %d Peng' % player)}
    ...
    def _chow(self, player, tile):
        ...
        self.packs[player].append(
            ('CHI', tile, int(self.curTile[1]) - num + 2))
        self.state = 0
        self.curPlayer = player
        for i in range(4):
            if i != player:
    ...
    def _promoteKong(self, player, tile):
        ...
        self.obs = {i: self.agents[i].request2obs(
            'Player %d BuGang %s' % (player, tile)) for i in range(4) if i != player}
    ...
    def _checkMahjong(self, player, isSelfDrawn=False, isAboutKong=False):
        try:
            ...
            self.obs = {i: self.agents[i].request2obs(
                'Player %d Hu' % player) for i in range(4)}
            if isSelfDrawn:
