...
if __name__ == "__main__":
    ...
    model_ckpt_paths = [
        r"C:\Courses\S6\MultiAgentSystem\projects\mahjong\output\ppo_finetune\model\model_0.pt",
        r"C:\Courses\S6\MultiAgentSystem\projects\mahjong\output\ppo_finetune\model\model_50000.pt",
        r"C:\Courses\S6\MultiAgentSystem\projects\mahjong\output\ppo_finetune\model\model_100000.pt",
        r"C:\Courses\S6\MultiAgentSystem\projects\mahjong\output\ppo_finetune\model\model_150000.pt",
    ]
    for model, ckpt_path in zip(models, model_ckpt_paths):
        if ckpt_path:
            model.load_state_dict(torch.load(ckpt_path))
    print("Evaluating the following checkpoints")
    for fpath in model_ckpt_paths:
        print(fpath)
    get_rewards(models, env, episodes=4000)
